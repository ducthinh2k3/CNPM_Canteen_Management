...
submitBtn.addEventListener('click', async () => {
    if (username.value !== '' && feedback.value !== '') {
        ...
        await saveReviewToDatabase(username.value, feedback.value, userRatingStar, time);
...
async function fetchReviews() {
    ...
    try {
        const res = await fetch(`http://localhost:3000/api/admin/dashboard/product-reviews/id=${MaSP}`)
